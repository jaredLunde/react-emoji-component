...
const isSurrogate = (str, pos) => 0xd800 <= str.charCodeAt(pos) && str.charCodeAt(pos) <= 0xdbff && 0xdc00 <= str.charCodeAt(pos + 1) && str.charCodeAt(pos + 1) <= 0xdfff; // Private function, gets a Unicode code point from a JavaScript UTF-16 string
...
const shouldBreak = (start, mid, end) => {
  let all = [start].concat(mid).concat([end]),
      ...
      next = end; // Lookahead termintor for:
  // GB10. (E_Base | EBG) Extend* ?	E_Modifier

  let eModifierIndex = all.lastIndexOf(E_Modifier);

  if (eModifierIndex > 1 && all.slice(1, eModifierIndex).every(split_ref) && [Extend, E_Base, E_Base_GAZ].indexOf(start) === -1) {
    return Break;
  ...
  if (rIIndex > 0 && all.slice(1, rIIndex).every(split_ref2) && Regional_Indicator !== previous) {
  ...
  if (previous === CR && next === LF) {
    return NotBreak;
      ...
      if (next === E_Modifier && mid.every(split_ref4)) {
        return Break;
      } else {
        return BreakStart;
      }
  ...
  if ([E_Base, E_Base_GAZ].indexOf(all[previousNonExtendIndex]) > -1 && all.slice(previousNonExtendIndex + 1, -1).every(_ref5) && next === E_Modifier) {
    return NotBreak;
...
const getGraphemeBreakProperty = code => {
  ...
  if (0x1D166 === code || // Mc       MUSICAL SYMBOL COMBINING SPRECHGESANG STEM
...
/* harmony default export */ var split = (splitGraphemes);
